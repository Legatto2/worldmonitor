  ...
  const installMiddleware = (server) => {
    server.middlewares.use('/api/cctv', async (req, res) => {
      try {
        ...
        // Austin publishes this verified "Image Unavailable" JPEG with HTTP 200.
        // Treat that exact operator placeholder as unavailable in the strict viewer.
        const knownPlaceholder =
          url.searchParams.get('liveOnly') === '1' &&
          source?.sourceKind === 'austin-open-data' &&
          upstreamImage?.ok &&
          createHash('sha256').update(upstreamImage.body).digest('hex') ===
            'db8d3ffca668cac202fd73df14bcc10e703b22f166903e8ff9937998d963e08e';
        if (knownPlaceholder) upstreamImage.ok = false;
        if (upstreamImage?.ok) {
          setHealth(cameraId, {
            status: 'ok',
            sourceKind: 'snapshot',
        ...
        // The public camera viewer must never label a fallback as a camera feed.
        if (url.searchParams.get('liveOnly') === '1') {
          res.writeHead(source ? 503 : 404, {
            'Content-Type': 'application/json',
            'Cache-Control': 'no-store',
          });
          res.end(
            JSON.stringify({ error: 'Operator camera snapshot unavailable' }),
          );
          return;
        }
