...
import { initShodanPanel } from '../ui/shodanPanel.js';
import { initUnifiedBridge } from '../integration/unifiedBridge.js';
...
export function createStandaloneTools({
  ...
  defer(initShodanPanel({ viewer }));
  const { styleManager, weatherEffects, cockpitCloudEffects } = controls;
  defer(initUnifiedBridge({ viewer, restoration: styleManager.initialRestorePromise }));
