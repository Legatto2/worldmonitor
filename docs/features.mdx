...
Streams are listed in `src/config/live-video-sources.ts`; `npm run live-video:check -- --all` reports any that stopped being live. A daily audit keeps one GitHub issue, "Live video sources: slots needing a replacement", that names each webcam or channel slot with nothing live, where it shows and why; maintainers check a replacement URL with `npm run live-video:check -- <url>` and paste it into that file. YouTube refuses embeds from GitHub's runners, so the audit plays YouTube entries through the residential proxy in the `LIVE_VIDEO_AUDIT_PROXY_URL` secret and stops with an error when that secret is missing. For a channel entry, the audit first reads the channel's `/live` page through the same proxy and checks the video the channel has live right now instead of the channel embed; when a pinned video ahead of the channel has died, the issue suggests that live video under "Live now", ready to paste. A failure the runner cannot tell apart from its own location or network, such as a region-locked channel refused outside its regions or an HLS playlist that a CDN edge kept serving unchanged, is listed under "Could not verify from the runner" instead of as a slot needing a replacement.
